...
> 旧プロジェクト `慎士班_デジコン` を全面的に再設計・整理した版。  
> 旧コードからの移動は行わず、**複製・リファクタリング**として進める。
...
│   └── assets/            ← 画像・音声ファイル
└── LiDAR/
    ├── lidarapp           ← LiDARアプリ（バイナリ、arm64）
    ├── footpoint.txt      ← ゲームと共有するファイル（LiDAR→ゲーム）
    └── scanarea.txt       ← スキャン領域設定
```
...
> 整合性確認(2026-06-22)の結果、最新・最終版は `main.cpp`（無印）と判明し、そこをベースに再構築します。  
> 最新 `main.cpp` では下記のいくつかは**既に対処済み**でした（防御の競合解消・横持ち対応など）。  
...
1. `lidarapp` を先に起動しないと `footpoint.txt` が更新されない
...
| ⏸ 検討 | 子ども大学日進向け転用（当面は鎌のまま・実施時に軽量プロップ＋敵マイルド化） | — |
